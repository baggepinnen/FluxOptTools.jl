...
Base.@kwdef mutable struct SLBFGS{F,Σ}
    ...
    λ::Float64 = 1.0 # regularizer, was ∈ [0.04, 1] in paper
    ρ::Bool = false # Algorithm indicator
    κ::Float64 = 0.5 # Step-length scaling factor ∈ (0,1)
    ᾱ::Float64 = 1.0 # maximum allowed step-length ∈ (0,1)  TODO: can be function of k
    α::Float64 = ᾱ
    fold::Float64 = Inf
    k::Int = 1 # Internal iteration index
    σ²::Σ = OnlineStats.Variance(weight=McclainWeight(0.1))
    xold::Vector{Float64}
    gold::Vector{Float64}
    f::F
    Y::Matrix{Float64}
...
lossfun, gradfun, fg!, p0 = FluxOptTools.optfuns(loss, pars)

opt = FluxOptTools.SLBFGS(lossfun, p0; m=3, λ=0.0001)
function train(opt, p0, iters=20)
    p = copy(p0)
    g = zeros(FluxOptTools.veclength(pars))
    trace = [loss()]
    for i = 1:iters
        g = gradfun(g, p)
        p = FluxOptTools.apply(opt, g, p)
